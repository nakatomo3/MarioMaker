...
void NumText::CreateTexture() {
	string numbers = "0123456789.-";
	TCHAR* c = (TCHAR*)(numbers.c_str());
...
void NumText::SetPosition(Vector3 newPos) {
	position = newPos;
}

Vector3 NumText::GetPosition() {
	return position;
}

void NumText::SetColor(D3DXVECTOR4 newColor) {
	color = newColor;
}

D3DXVECTOR4 NumText::GetColor() {
	return color;
}

void NumText::SetText(float _number) {
	number = _number;
	float num = number;
...
void NumText::SetFontSize(int _size) {
	fontSize = _size;
}

int NumText::GetFontSize() {
	return fontSize;
}

void NumText::SetMinDigit(int _digit) {
	digit = _digit;
}
