...
SaveManager::SaveManager() {
	LogWriter::Log("セーブマネージャーが正常に初期化されました");
}
...
bool SaveManager::SaveAll(const char* fileName) {
	...
	if (file == NULL) {
		LogWriter::LogWorning("ファイルが見つかりませんでしたので、新規作成しました\n");
...
bool SaveManager::LoadAll(const char* fileName) {
	...
	for (int i = 0; i < saveCount; i++) {
		...
		switch (classID) {
			default:
				LogWriter::LogError("CLASS_IDを登録してください\n");
				isSuccess = false;
				break;
			case INT:
	...
	if (isSuccess == true) {
		LogWriter::Log("ロードが正常に完了しました\n");
	} else {
		LogWriter::LogWorning("ロードが正常に完了しませんでした\n");
	}
	fclose(file);
	return isSuccess;
}
